...
def compute_stft(
    ...
    input_length = int(x_arr.size)
    if input_length < 2:
        raise ValueError(
            f"STFT input too short: requires at least 2 samples, got {input_length}."
        )

    requested_nperseg = max(2, int(round(float(params.window_length_s) * float(fs))))
    requested_nfft = max(8, requested_nperseg)
    nperseg = min(requested_nperseg, input_length)
    noverlap = int(round(nperseg * float(params.overlap)))
    noverlap = min(max(noverlap, 0), nperseg - 1)
    nfft = max(requested_nfft, nperseg)
